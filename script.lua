...
local function findPrompt(nearest)
    ...
    for _,plot in ipairs(plots:GetChildren()) do
        ...
        for _,pod in ipairs(pods:GetChildren()) do
            pcall(function();local base=pod:FindFirstChild("Base");local spawn=base and base:FindFirstChild("Spawn")
                if spawn then
    ...
    C.aim=RS.Heartbeat:Connect(function()
        ...
        for _,p in ipairs(game:GetService("Players"):GetPlayers()) do
            if p~=Pl and p.Character then
                local eh=p.Character:FindFirstChild("HumanoidRootPart");local h2=p.Character:FindFirstChildOfClass("Humanoid")
...
RS.Heartbeat:Connect(function()
    ...
    hrp.AssemblyLinearVelocity=Vector3.new(md.X*Cfg.StealSpeed,hrp.AssemblyLinearVelocity.Y,md.Z*Cfg.StealSpeed);end)
local function clearProx() for _,h in pairs(proxHL) do pcall(function() h:Destroy() end) end;proxHL={} end
-- ── GUI ──
local BG=Color3.fromRGB(15,15,20);local CARD=Color3.fromRGB(22,22,30);local GRN=Color3.fromRGB(0,200,100);local WHT=Color3.fromRGB(255,255,255);local GRY=Color3.fromRGB(130,130,150);local DRK=Color3.fromRGB(10,10,16)
...
leftP.BackgroundColor3=BG;leftP.BackgroundTransparency=0.05;leftP.BorderSizePixel=0;leftP.ZIndex=10;corner(10,leftP);Instance.new("UIStroke",leftP).Color=Color3.fromRGB(35,35,50);drag(leftP)
...
local b=Instance.new("TextButton",lScroll);b.Size=UDim2.new(1,0,0,28);b.BackgroundColor3=CARD;b.BackgroundTransparency=0.2;b.BorderSizePixel=0
    b.Text=label;b.TextColor3=WHT;b.Font=Enum.Font.GothamBold;b.TextSize=11;b.ZIndex=12
    corner(6,b);b.MouseButton1Click:Connect(cb);b.MouseEnter:Connect(function() tw(b,{BackgroundTransparency=0}) end)
...
end;local proximityLbl=Instance.new("TextLabel",leftP);proximityLbl.Size=UDim2.new(1,-8,0,12);proximityLbl.Position=UDim2.new(0,4,0,22)
...
mkLeftBtn("Kick (Y)",function() Pl:Kick("Secret Hub") end);mkLeftBtn("Reset (X)",function() local h=getHum();if h then h.Health=0 end end)
mkLeftHead("SETTINGS");mkLeftBtn("⚙ Settings",function() end)
...
main.BackgroundColor3=BG;main.BackgroundTransparency=0.05;main.BorderSizePixel=0;main.ClipsDescendants=true;main.ZIndex=10;corner(12,main);Instance.new("UIStroke",main).Color=Color3.fromRGB(35,35,50)
...
Instance.new("TextLabel",mTitleBar):ClearAllChildren();local mTitle=Instance.new("TextLabel",mTitleBar);mTitle.Size=UDim2.new(1,-80,0,24);mTitle.Position=UDim2.new(0,12,0,4)
...
xBtn.MouseButton1Click:Connect(function() sg:Destroy() end);drag(main,mTitleBar)
-- SIDEBAR
local sidebar=Instance.new("Frame",main);sidebar.Size=UDim2.new(0,70,1,-44);sidebar.Position=UDim2.new(0,0,0,44);sidebar.BackgroundColor3=DRK;sidebar.BorderSizePixel=0;sidebar.ZIndex=11
...
-- CONTENT
local ca=Instance.new("Frame",main);ca.Size=UDim2.new(1,-70,1,-44);ca.Position=UDim2.new(0,70,0,44);ca.BackgroundTransparency=1;ca.ZIndex=11
local panels={}
for _,name in ipairs(TABS) do
    local p=Instance.new("ScrollingFrame",ca);p.Size=UDim2.new(1,0,1,0);p.BackgroundTransparency=1;p.BorderSizePixel=0
p.ScrollBarThickness=3;p.ScrollBarImageColor3=GRN;p.CanvasSize=UDim2.new(0,0,0,0);p.AutomaticCanvasSize=Enum.AutomaticSize.Y;p.ZIndex=12;p.Visible=name=="Main"
...
mkBtn2(panels["Main"],"Kick (Y)",RED,function() Pl:Kick("Secret Hub") end);mkBtn2(panels["Main"],"Reset (X)",Color3.fromRGB(100,40,40),function() local h=getHum();if h then h.Health=0 end end)
-- STEALS TAB
mkSection(panels["Steals"],"AUTO UNLOCK");mkToggle(panels["Steals"],"Auto Unlock on Steal","AutoUnlock",nil,nil)
mkToggle(panels["Steals"],"Show Unlock Buttons HUD","ShowHUD",nil,nil);mkSection(panels["Steals"],"AUTOMATION")
mkToggle(panels["Steals"],"Auto Invis During Steal","AutoInvis",nil,nil);mkToggle(panels["Steals"],"Auto TP on Failed Steal","AutoTPFail",nil,nil)
mkToggle(panels["Steals"],"Auto TP Priority Mode","AutoTPPriority",nil,nil);mkToggle(panels["Steals"],"Auto-Kick on Steal","AutoKick",nil,nil)
-- MISC TAB
...
mkToggle(panels["Misc"],"Aimbot","Aimbot",startAimbot,stopAimbot);mkToggle(panels["Misc"],"Speed [E]","SpeedEnabled",nil,nil)
...
corner(10,stPanel);Instance.new("UIStroke",stPanel).Color=Color3.fromRGB(35,35,50);drag(stPanel)
Instance.new("TextLabel",stPanel):ClearAllChildren();local stTitle=Instance.new("TextLabel",stPanel);stTitle.Size=UDim2.new(1,0,0,24);stTitle.BackgroundTransparency=1
...
stScroll.CanvasSize=UDim2.new(0,0,0,0);stScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y;stScroll.ZIndex=11;Instance.new("UIListLayout",stScroll).Padding=UDim.new(0,2)
local stLabels={}
for i=1,5 do
...
qPanel.BackgroundColor3=BG;qPanel.BackgroundTransparency=0.05;qPanel.BorderSizePixel=0;qPanel.ZIndex=10;corner(10,qPanel);Instance.new("UIStroke",qPanel).Color=Color3.fromRGB(35,35,50);drag(qPanel)
...
Instance.new("UIListLayout",qScroll).Padding=UDim.new(0,3);Instance.new("UIPadding",qScroll).PaddingTop=UDim.new(0,4)
local function mkQRow(label,tKey,onFn,offFn)
    ...
    clk.MouseButton1Click:Connect(function()
        ...
        tw(tb,{BackgroundColor3=isOn and GRN or Color3.fromRGB(40,40,60)});tw(knob,{Position=isOn and UDim2.new(1,-16,0.5,-7)or UDim2.new(0,2,0.5,-7)})
        if isOn and onFn then onFn()end;if not isOn and offFn then offFn()end
    end)
end
mkQRow("Auto steal","AutoSteal",startSteal,stopSteal);mkQRow("Steal highest","StealHighest",startSteal,stopSteal)
mkQRow("Steal priority","StealPriority",startSteal,stopSteal);mkQRow("Steal nearest","StealNearest",startSteal,stopSteal)
...
corner(10,invPanel);Instance.new("UIStroke",invPanel).Color=Color3.fromRGB(35,35,50);drag(invPanel)
...
Instance.new("UIListLayout",invScroll).Padding=UDim.new(0,3);Instance.new("UIPadding",invScroll).PaddingTop=UDim.new(0,4)
...
local isOn=tKey and T[tKey] or false;valBtn.BackgroundColor3=isOn and GRN or Color3.fromRGB(140,40,40);valBtn.BorderSizePixel=0
...
isOn=not isOn;if tKey then T[tKey]=isOn end;valBtn.Text=isOn and"ON"or"OFF";tw(valBtn,{BackgroundColor3=isOn and GRN or Color3.fromRGB(140,40,40)})
        if isOn and onFn then onFn()end;if not isOn and offFn then offFn()end
    end)
end
mkInvRow("Toggle","InvisToggle",nil,nil);mkInvRow("Fix lagback","FixLagback",nil,nil)
mkInvRow("Inv on steal","InvOnSteal",nil,nil);mkInvRow("Unlock on steal","UnlockOnSteal",nil,nil)
mkInvRow("Aimbot","Aimbot",startAimbot,stopAimbot);mkInvRow("Desync","Desync",nil,nil)
...
corner(10,ssPanel);Instance.new("UIStroke",ssPanel).Color=Color3.fromRGB(35,35,50);drag(ssPanel)
...
ssBtn.MouseButton1Click:Connect(function();T.SpeedEnabled=not T.SpeedEnabled;ssStatus.Text=T.SpeedEnabled and"ENABLED"or"DISABLED";ssStatus.TextColor3=T.SpeedEnabled and GRN or RED
end)
-- ── ADMIN PANEL ──
...
corner(10,adminF);Instance.new("UIStroke",adminF).Color=Color3.fromRGB(35,35,50);drag(adminF)
...
adminList.BackgroundTransparency=1;adminList.BorderSizePixel=0;adminList.ScrollBarThickness=2;adminList.ScrollBarImageColor3=GRN;adminList.CanvasSize=UDim2.new(0,0,0,0);adminList.ZIndex=11
    ...
    for _,p in ipairs(game:GetService("Players"):GetPlayers()) do
        if p~=Pl and p.Character then
            ...
            if eh and(eh.Position-hrp.Position).Magnitude<=Cfg.ProxRad then
                table.insert(nearby,p);local hl=Instance.new("SelectionBox");hl.Adornee=p.Character;hl.Color3=Color3.fromRGB(0,200,255)
                hl.LineThickness=0.05;hl.SurfaceTransparency=0.8;hl.SurfaceColor3=Color3.fromRGB(0,100,200);hl.Parent=Pl.PlayerGui;table.insert(proxHL,hl)
            end
        end
end;proximityLbl.Text="Proximity: "..(T.ProxCircle and"ON"or"OFF").." | "..#nearby.." nearby";adminF.Visible=#nearby>0
...
end;adminList.CanvasSize=UDim2.new(0,0,0,#nearby*46);end)
-- ── FPS BAR ──
...
local fr,lt=0,tick();RS.RenderStepped:Connect(function()
    fr=fr+1;if tick()-lt>=1 then local fps=fr;fr=0;lt=tick()
local ok,ping=pcall(function() return math.floor(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue()) end);fpsLbl.Text="FPS: "..fps.."  PING: "..(ok and ping or"?").."ms"
    end
end)
-- ── TOGGLE ──
minBtn.MouseButton1Click:Connect(function() guiVisible=not guiVisible;main.Visible=guiVisible end);UIS.InputBegan:Connect(function(inp,gpe)
